...
def answer_2():
    ...
    state = move.LocalXY(state,pi*0.125,0.0,[2])
    state = move.GlobalCZ(atom_state=state)
    # state = local_RX(atom_state=state, angle = -pi *.125, indices=[0])
    state = move.LocalXY(state,-pi*0.125,0.0,[2])
    state = move.GlobalCZ(atom_state=state)
    # state = local_RX(atom_state=state, angle = pi *.25, indices=[0])
    state = move.LocalXY(state,pi*0.25,0.0,[2])
    ...
    state = move.LocalXY(state,pi*0.125,0.0,[3])
    state = move.GlobalCZ(atom_state=state)
    state.gate[[4]] = move.Move(state.gate[[2]])
    state.gate[[5]] = move.Move(state.gate[[0]])
    # state = local_RX(atom_state=state, angle = pi *.25, indices=[4])
    state = move.LocalXY(state,pi*0.25,0.0,[4])
