...
def global_H(atom_state:move.core.AtomState):
    state = move.GlobalXY(atom_state=atom_state,x_exponent=-1.0/2,axis_phase_exponent=1.0)
...
def local_H(atom_state:move.core.AtomState,indices):
    state = move.GlobalXY(atom_state=atom_state,x_exponent=1.0/4,axis_phase_exponent=1.0/2)
    state = move.LocalRz(atom_state=state,phi=1.0,indices=indices)
    state = move.GlobalXY(atom_state=state,x_exponent=-1.0/4,axis_phase_exponent=1.0/2)
    return state
...
def local_T(atom_state:move.core.AtomState,indices, dag=False):
    state = move.LocalRz(atom_state=atom_state,phi=1.0/4 * (-1**dag),indices=indices)
    return state

@move.vmove()
def global_T(atom_state:move.core.AtomState):
    state = move.GlobalRz(atom_state=atom_state,phi=1.0/4)
    return state

@move.vmove()
def local_S(atom_state:move.core.AtomState,indices):
    state = move.LocalRz(atom_state=atom_state,phi=1.0/2,indices=indices)
    return state

@move.vmove()
def global_S(atom_state:move.core.AtomState):
    state = move.GlobalRz(atom_state=atom_state,phi=1.0/2)
    return state
